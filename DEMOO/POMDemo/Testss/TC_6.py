...
class RegisterTest(unittest.TestCase):
    ...
    def test_case1(self):
        ...
        color = driver.find_element(By.XPATH,".//*[@id='genterWrapper']//label[contains(text(),'Female')]").value_of_css_property('border-color')       
        # print(color)
        hex = Color.from_string(color).hex
        print(hex)
        color_fail = "#dc3545"
        
        gender = driver.find_element(By.XPATH,".//*[@id='genterWrapper']//label[contains(text(),'Female')]")
        gender_select = gender.text
        print(gender_select)
        
        value = driver.find_element(By.XPATH,"//td[contains(text(),'Female')]")
        value_gender = value.text
        print(value_gender)
        
        if ( gender_select == value_gender):
            print("Testcase Pass")
        else:
            print("Testcase Fail")
